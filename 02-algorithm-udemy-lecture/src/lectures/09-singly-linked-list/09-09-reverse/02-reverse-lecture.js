...
export class SinglyLinkedList {
  ...
  reverse() {
    ...
    // i = 0; i < this.length; i++ 반복하여,
    for (let i = 0; i < this.length; i++) {
      // 현재 Node 의 next 를 원본 nextNode 변수로 만든다.
      const originNextNode = currentNode.next;

      // 현재 Node 의 next 를 이전 Node 로 변경하기
      currentNode.next = prevNode;
      // 이전 Node 를 현재 Node 로 변경한다.
      prevNode = currentNode;
      // 현재 Node 를 원본 nextNode 로 변경한다.
